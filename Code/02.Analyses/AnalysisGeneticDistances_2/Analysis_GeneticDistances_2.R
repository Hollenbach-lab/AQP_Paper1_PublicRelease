...
# Analysis
analysisName = "Analysis_GeneticDistances_2"
...
nAncestrySelectedMin = 1 # Min
nAncestrySelectedMax = 1 # Max
...
# 04. Source codes --------------------------------------------------------
source("Code/02.Analyses/AnalysisGeneticDistances_2/Edwards_Helpers.R")
...
PopAnc$nAnc <- rowSums(PopAnc %>% select(-studyid))
PopAnc <- PopAnc %>% filter(nAnc >= nAncestrySelectedMin, nAnc <= nAncestrySelectedMax)
PopAnc$ancCombi <- apply(sapply(colnames(PopAnc %>% select(-c(studyid, nAnc))), function(colName) {
  res <- rep("*", nrow(PopAnc))
...
if (nPerm == 0) { cat("No permutations wanted\n") 
  ...
  for (coupleIndex in 1:nrow(popCouples)) {
    ...
    permDistances <- pbmcapply::pbmclapply(X = 1:nPerm, genind = coupleGenind, FUN = perm.pop, mc.cores = 20)
