...
# Analysis
analysisName = "Analysis_GeneticDistances_1"

# Response Categories and Modifiers
responseCateg = "Ancestries"
modifier = "Races"
...
nRacesSelectedMax = 7 # Max
# Number of selected ancestries
nAncestrySelectedMin = 2 # Min
nAncestrySelectedMax = 2 # Max

# Race and/or Ancestry to focus the analysis on
raceFocus = NA # NA, "Asian" ...
ancestryFocus = c("WesternEurope & AmericanIndian") # NA, "EastAsia" ...
...
# 04. Source codes --------------------------------------------------------
source("Code/02.Analyses/AnalysisGeneticDistances_1/Edwards_Helpers.R")
...
PopAnc$nAncestries <- rowSums(PopAnc %>% select(-studyid))
PopAnc <- PopAnc %>% filter(nAncestries >= nAncestrySelectedMin, nAncestries <= nAncestrySelectedMax)
# PopAnc <- PopAnc %>% filter(WesternEurope == 1, AmericanIndian == 1, SouthernEurope == 0, EasternEurope == 0, Scandinavia == 0,
                            # EastAsia == 0, SouthAsia == 0, SoutheastAsia == 0, PacificIslands == 0, Caribbean == 0, CentralorSouthAmerica == 0,
                            # NorthernAfrica == 0, SubSaharanAfrica == 0, AfricanAmerican == 0, Unknown == 0)
PopAnc <- PopAnc %>% filter(AmericanIndian == 1, WesternEurope == 1)

PopAnc$ancCombi <- apply(sapply(colnames(PopAnc %>% select(-studyid, -nAncestries)), function(colName) {
  res <- rep("*", nrow(PopAnc))
  boolIndex <- as.logical(PopAnc[[colName]]) & !is.na(PopAnc[[colName]])
  res[boolIndex] <- colName
  res
}), 1, paste, collapse = "_")
PopAnc$ancCombi <- gsub("[*]_", "", PopAnc$ancCombi)
PopAnc$ancCombi <- gsub("_[*]", "", PopAnc$ancCombi)
PopAnc <- PopAnc %>% select(studyid, Desc_Ancestries = ancCombi)

PopData <- PopRace %>% inner_join(PopAnc, by = "studyid")

table(PopData$Desc_Races, PopData$Desc_Ancestries, useNA = "ifany")
